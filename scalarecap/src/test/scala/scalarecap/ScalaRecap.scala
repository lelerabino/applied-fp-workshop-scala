...
class ScalaRecap extends munit.FunSuite {
  ...
  case class Person(name: String, age: Int) {
    def apply(greeting: String):String = {
      s"$greeting mi chiamo $name!"
    }

    def makeOlder(increase:Int): Person = Person(name, age + increase)

    def makeYounger(implicit decrease: Int): Person = makeOlder(-decrease)
  }

  object Person {
    def create(nameWithAge: String): Person = {
      val tokens = nameWithAge split ';' take 2
      val name = tokens(0)
      val age = tokens(1).toInt
      Person(name, age)
    }

    val apply: String => Person = (x:String) => create(x)
  }
